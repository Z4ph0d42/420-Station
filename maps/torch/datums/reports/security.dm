...
/datum/computer_file/report/recipient/sec/restraining/generate_fields()
	..()
	add_field(/datum/report_field/text_label/header, "SEV Torch Security Department")
	add_field(/datum/report_field/text_label/instruction, "To be filled out by the Chief of Security, First Mate, or The Captain. Report must be signed and submitted for the order to be considered valid. Any paper copies must be stamped.")
